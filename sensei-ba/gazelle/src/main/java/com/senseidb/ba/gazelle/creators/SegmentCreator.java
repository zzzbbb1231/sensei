...
public class SegmentCreator {
  ...
  public static GazelleIndexSegmentImpl readFromAvroFileOnHadoop(String avroFilePath, FileSystem fs) throws IOException {
    MetadataCreator creator = new MetadataCreator();
    Path path = new Path(avroFilePath);
    FSDataInputStream inputStream1 = null;
    FSDataInputStream inputStream2 = null;
    try {
      Map<String, GazelleForwardIndexImpl> ret = new HashMap<String, GazelleForwardIndexImpl>();
      DatumReader<GenericRecord> datumReader = new GenericDatumReader<GenericRecord>();
      inputStream1 = fs.open(path);
      ...
      for (int j = 0; j < columnTypes.length; j++) {
        if (!creators[j].isSorted()) {
          ...
          // dictionaries[j].size());
          intArrays[j] =
              new CompressedIntArray(count, CompressedIntArray.getNumOfBits(dictionaries[j].size()), getByteBuffer(
                  count, dictionaries[j].size()));
        } else {
          sortedForwardIndexes[j] =
              new SortedForwardIndexImpl(dictionaries[j], new int[dictionaries[j].size()],
                  new int[dictionaries[j].size()], count, creator.createMetadata(columnNames[j], dictionaries[j],
                      columnTypes[j], count, true));
        }
      }
      dataFileReader.close();
      datumReader = new GenericDatumReader<GenericRecord>();
      inputStream2 = fs.open(path);
        ...
        for (int j = 0; j < columnTypes.length; j++) {
          if (!creators[j].isSorted()) {
            try {
              ...
              builder.append("\nbufferSize = "
                  + CompressedIntArray.getRequiredBufferSize(intArrays[j].getCapacity(),
        ...
        for (int j = 0; j < columnTypes.length; j++) {
            if (!creators[j].isSorted()) {
                try {
                  ...
                  System.out.println("capacity = " + intArrays[j].getCapacity());
                  System.out.println("bitSize = " + CompressedIntArray.getNumOfBits(dictionaries[j].size()));
                  System.out.println("bufferSize = " + CompressedIntArray.getRequiredBufferSize(intArrays[j].getCapacity(), CompressedIntArray.getNumOfBits(dictionaries[j].size())));
