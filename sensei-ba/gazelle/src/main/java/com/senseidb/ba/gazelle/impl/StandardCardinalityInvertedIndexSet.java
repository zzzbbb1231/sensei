...
public class StandardCardinalityInvertedIndexSet extends DocIdSet {
   ...
   * 
   * @param id
   *          -> The DocID you want to add to the set.
   * @throws IOException
   *           -> Comes from Kamikaze's PForDelta implementation
   ...
   * 
   * @param forwardIndex
   *          -> Needed to fetch the reader
   * @param dictValue
   *          -> Needed for sequential iteration
   * @throws IOException
   */
  ...
  private interface ForwardIndexReader {
     ...
     * 
     * @param index
     *          -> We grab the first row with the specified value starting with one HIGHER than this index
  ...
  class GazelleInvertedIndexIterator extends DocIdSetIterator {
     ...
     * 
     * @param lowerBound
     *          -> We should start iterating from at least this value
     * @param target
     *          -> This is the value that target was called with
     * @throws IOException
     *           -> Comes from kamikaze's API.
     */
