...
public class ForwardIndexBackedByArray implements ForwardIndex {
  ...
  public void initByLongValues(List<Long> values) {
    ...
    TermLongList termLongList =
        new TermLongList(longAVLTreeSet.size(), DefaultSenseiInterpreter.DEFAULT_FORMAT_STRING_MAP.get(long.class));
    ...
    for (int i = 0; i < values.size(); i++) {
      int dictIndex =
          values.get(i) != null && long2IntMap.containsKey(values.get(i)) ? long2IntMap.get(values.get(i)) : 0;
  ...
  public void initByIntValues(List<Integer> values) {
    ...
    TermIntList termIntList =
        new TermIntList(intAVLTreeSet.size(), DefaultSenseiInterpreter.DEFAULT_FORMAT_STRING_MAP.get(int.class));
    ...
    for (int i = 0; i < values.size(); i++) {
      int dictIndex =
          values.get(i) != null && int2IntMap.containsKey(values.get(i)) ? int2IntMap.get(values.get(i)) : 0;
      forwardIndex[i] = dictIndex;
      freqs[dictIndex]++;
    }
  }
  ...
  public void initByStringValues(List<String> values) {
    ...
    for (int i = 0; i < values.size(); i++) {
      int dictIndex =
          values.get(i) != null && obj2IntMap.containsKey(values.get(i)) ? obj2IntMap.get(values.get(i)) : 0;
      forwardIndex[i] = dictIndex;
      freqs[dictIndex]++;
    }
  }
